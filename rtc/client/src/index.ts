...
import { makeConnection } from "./peer";
...
export const babylonInit = async () => {
    ...
    if(p.initiator) { //server bumps up the ball every once in a while
    ...
    engine.runRenderLoop(function () {
        ...
        if(renderCounter % FRAMES_PER_UPDATE === 0) {
            /*console.log(renderCounter);*/

            if(p.initiator) {
                updateServer(scene, p);
            } else {
                updateClient(scene);
            }
        }
